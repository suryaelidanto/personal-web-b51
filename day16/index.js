...
app.post('/update-blog', upload.single("image"), updateBlog)
...
async function blog(req, res) {
    const query = `SELECT blogs.id, blogs.title, blogs.content, blogs.image, 
    users.name AS author, blogs."createdAt", blogs."updatedAt" FROM blogs LEFT JOIN users ON
    blogs."authorId" = users.id`
    const obj = await sequelize.query(query, { type: QueryTypes.SELECT })
    // const data = await blogModel.findAll()
    // console.log("data", data)
    console.log("data blog", obj)
...
async function addBlog(req, res) {
    ...
    const query = `INSERT INTO blogs(title, content, image, "authorId") VALUES ('${title}', '${content}','${image}','${authorId}')`
...
async function updateBlog(req, res) {
    ...
    if (!image) {
        const query = `SELECT blogs.id, blogs.title, blogs.content, blogs.image, 
        users.name AS author, blogs."createdAt", blogs."updatedAt" FROM blogs LEFT JOIN users ON
        blogs."authorId" = users.id WHERE blogs.id=${id}`
        const obj = await sequelize.query(query, { type: QueryTypes.SELECT })
        image = obj[0].image
    }
